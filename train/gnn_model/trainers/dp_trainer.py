...
def train(device, dataset, file_config):
    ...
    train_loader = DataListLoader(train_dataset,  
                                  batch_size = config_train["batch_size"] * torch.cuda.device_count(),  
                                  shuffle = True, 
    ...
    # Build the model
    DSSModel = model.DeepStatisticalSolver(config_model)
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode = 'min', factor = 0.1, patience=10, min_lr=1.e-7)
    ...
    for epoch in range(init_epoch, config_train["max_epochs"]):
        ...
        for i, train_batch in enumerate(train_loader):
            ...
            cumul_loss += loss.mean().item()
            cumul_residual_loss += loss_dic["residual_loss"][str(config_model["k"])].mean().item()
            cumul_mse_loss += loss_dic["mse_loss"][str(config_model["k"])].mean().item()
            cumul_rel_loss += loss_dic["rel_loss"][str(config_model["k"])].mean().item()

            # Accumulate running loss and print every 25%
            run_loss += loss.item()
            run_residual_loss += loss_dic["residual_loss"][str(config_model["k"])].mean().item()
            run_mse_loss += loss_dic["mse_loss"][str(config_model["k"])].mean().item()
            run_rel_loss += loss_dic["rel_loss"][str(config_model["k"])].mean().item()
        ...
        with torch.no_grad():
            ...
            for val_batch in val_loader:
                ...
                cumul_val_residual_loss += loss_dic["residual_loss"][str(config_model["k"])].mean().item()
                cumul_val_mse_loss +=  loss_dic["mse_loss"][str(config_model["k"])].mean().item()
                cumul_val_rel_loss +=  loss_dic["rel_loss"][str(config_model["k"])].mean().item()
        ...
        scheduler.step(hist_val["loss"][-1])
