...
def train(world_size, rank, local_rank, dataset, file_config):
    ...
    # Build the model
    DSSModel = model.DeepStatisticalSolver(config_model).to(local_rank)
    DSSModel = DistributedDataParallel(DSSModel, device_ids=[local_rank])

    if rank == 0 : 

        with open(os.path.join(config_train["path_logs"],"training_info.csv"), 'w') as f : 
            f.write(f"Number of GPU used : {torch.cuda.device_count()}, Device : {torch.cuda.get_device_name(torch.cuda.current_device)}")
            f.write("\n")
    ...
    for epoch in range(init_epoch, config_train["max_epochs"]):
        ...
        for i, train_batch in enumerate(train_loader):
            ...
            # put batch on gpu
            train_batch = train_batch.to(local_rank)
        ...
        if rank == 0 :
            ...
            with torch.no_grad():
                ...
                for i, val_batch in enumerate(val_loader):
                    ...
                    val_batch = val_batch.to(local_rank)
            ...
            scheduler_metric = torch.tensor(cumul_val_loss / len(val_loader), device = 'cuda')
        
        else : 

            scheduler_metric = torch.tensor(0.0, device = 'cuda')

        dist.broadcast(tensor=scheduler_metric, src=0)

        scheduler.step(scheduler_metric.item())
        
        print(f"Learning Rate Epoch {epoch}, rank {rank} : {optimizer.param_groups[0]['lr']}")
